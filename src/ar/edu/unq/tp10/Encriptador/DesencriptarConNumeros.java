...
private Map<String, String> abecedarioNumerico;
	...
	public String desencriptar(String word) {
        String abecedario = " abcdefghijklmnnopqrstuvwxyz";
        String textoDesencriptado = word; 
		
        for (int i = 0; i < abecedario.length() ; i++) {
        	Integer lugarDeLetra = abecedario.length() - 1 - i;
        	textoDesencriptado = textoDesencriptado.replace(lugarDeLetra+",", abecedario.charAt(lugarDeLetra)+"");
        }
        textoDesencriptado = textoDesencriptado.replace((word.length() - 1)+"", abecedario.charAt(word.length() - 1)+"");
